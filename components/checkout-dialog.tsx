...
export const CheckoutDialog = ({
  ...
  const handleMpesaSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/mtcm-foundation/paystack/mpesa`, {
        method: 'POST',
        ...
        {succeeded ? (
          <div className="space-y-6 py-8 text-center">
            ...
            <p className="text-lg text-[#282828]/80">
              Your support helps the Michael Trufosa Clarice Mugenya Foundation keep this work moving.
            </p>
            {statusMessage && (
              <p className="text-sm text-[#282828]/60">{statusMessage}</p>
            )}
            <button
              onClick={handleClose}
          ...
          <div className="space-y-8">
            ...
            {method !== 'cheque' && (
              <div>
                <label htmlFor="donation-amount" className="mb-2 block text-xs font-bold uppercase tracking-widest">
                  Amount (KES)
                </label>
                <input
                  id="donation-amount"
                  ...
                  className="w-full border-b-2 border-[#282828]/20 py-4 text-xl font-bold transition-colors focus:border-[#fe0000] focus:outline-none md:text-2xl"
                  value={amount}
            ...
            <div className={method === 'mpesa' ? 'block' : 'hidden'}>
              <form onSubmit={handleMpesaSubmit} className="space-y-6">
                <div>
                  <label htmlFor="mpesa-email" className="mb-2 block text-xs font-bold uppercase tracking-widest">
                    Email
                  </label>
                  <input
                    id="mpesa-email"
                ...
                <div>
                  <label htmlFor="mpesa-phone" className="mb-2 block text-xs font-bold uppercase tracking-widest">
                    M-Pesa phone
                  </label>
                  <input
                    id="mpesa-phone"
                    ...
                    required={method === 'mpesa'}
                  />
                  <p className="mt-2 text-xs text-[#282828]/50">Format: 072xxxxxxx or 25472xxxxxxx. Minimum 10 KES.</p>
                </div>

                {mpesaMessage && (
                  <p className={`text-sm ${mpesaMessage.toLowerCase().includes('check your phone') ? 'text-[#282828]' : 'text-red-600'}`}>
                    {mpesaMessage}
                  </p>
                )}
                  ...
                  <div
                    className={`rounded-md p-3 text-sm font-medium ${
                      paymentStatus === 'success'
                        ? 'bg-green-50 text-[#282828]'
                        : paymentStatus === 'failed'
                          ? 'bg-red-50 text-red-700'
                          : 'bg-[#fe0000]/10 text-[#282828]'
                    }`}
                  >
                    {statusMessage}
                  </div>
